var Page = {
    ...
    my_grid_config: {
        store_config: {
            fields: ['UnitClassId', 'UnitClassName', 'UnitClassCode', 'UnitClassAlias', 'DescriptionInfo']
        },
        ...
        grid_config: {
            columns: {
                ...
                //{
                //    text: '编码',
                //    dataIndex: 'UnitClassCode',
                //    flex: 0.225
                //},
                //{
                //    text: '简写',
                //    dataIndex: 'UnitClassAlias',
                //    flex: 0.225
                //},
                {
                    text: '备注',
    ...
    init: function () {
        ...
        $('#query').click(function () {
            that.my_grid.query({ UnitClassName: $('#search-name').val() });
            that.clear();
        });
        ...
        // 页面加载后先查询一次
        this.my_grid.query({ UnitClassName: '' });
        ...
        $('#export').click(function () {
            that.my_grid.exportExcel('单位类型管理');
        })
    ...
    on_grid_row_delete_clicked: function (record) {
        var that = this;
        var data = { UnitClassId: record.UnitClassId };
        Util.confirm('确认要删除名称为' + record.UnitClassName + '的记录吗?', function () {
            $.ajax({
                url: '/api/UnitClass/Delete',
                type: 'delete',
                data: data,
                success: function (data) {
