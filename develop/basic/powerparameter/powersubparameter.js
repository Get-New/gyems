...
var Page = {
	...
	my_grid_config: function () {
		return {
			...
			url: '/api/BasicSubefficacious/GetPageWithName',
			ps: 8,
	...
	init: function () {
		...
		$('#query').click(function () {
			that.my_grid.query({ EfficaciousId: that.current_tree_select_id, EfficaciousType: '动力调度' });
			that.clear();
		});
		...
		$('#export').click(function () {
			that.my_grid.exportExcel('动力调度单元');
		})
	...
	get_effect_history: function (id) {
		...
		$.ajax({
			...
			dataType: 'json',
			async: false,
			data: { EfficaciousId: id, DescriptionInfo: '额定功率,偏差上限,偏差下限' },
	...
	tree_config: {
		url: '/api/BasicEfficacious/GetTree?EfficaciousType=' + escape('动力调度'),
		root_name: '动力调度单元',
	...
	tree_init: function () {
		...
		$('#refresh').click(function () {
			...
			that.my_grid.query({ EfficaciousId: '', EfficaciousType: '动力调度' });
		});
	},
	...
	tree_node_click: function (id, name) {
		...
		this.my_grid.query({ EfficaciousId: id, EfficaciousType: '动力调度' });
	}
}