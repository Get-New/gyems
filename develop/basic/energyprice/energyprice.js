var Page = {
	...
	init: function () {
		...
		$('#query').click(function () {
			that.my_grid.query({ EnergyMediumId: that.current_tree_select_id, EfficaciousType: '价格系数' });
			that.clear();
		});
		...
		$('#export').click(function () {
			that.my_grid.exportExcel('价格单元管理');
		})
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (!EnergyUnitList || !EnergyUnitList.length) {
					$('#EfficaciousUnit').append("<option value='-1'>没有价格单位可供选择</option>");
	...
	tree_init: function () {
		...
		$('#refresh').click(function () {
			...
			that.my_grid.query({ EnergyMediumId: '', EfficaciousType: '价格系数' });
		});
	},
	...
	tree_node_click: function (id, name) {
		...
		this.my_grid.query({ EnergyMediumId: id, EfficaciousType: '价格系数' });
		this.prepare_select_source_EfficaciousUnit();
	}
}