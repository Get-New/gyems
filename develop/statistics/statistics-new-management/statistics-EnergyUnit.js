var Page = {
    ...
    //默认介质ID 天然气ID为1d22868d659ed3bA27bc86aa
    default_energy_medium_id: '1d22868d659ed3bA27bc86aa',
    ...
    init: function () {
        ...
        //初始化上方介质筛选框 默认选中天然气default_energy_medium_id
        that.get_select_medium();
        $('#Medium-selector').val(that.default_energy_medium_id);
        ...
        $('#query').click(function () {
            that.my_grid.query({ MeasurePropertyName: $('#search-name').val(), ModelBaseID: that.current_tree_select_id, EnergyMediumId: $('#Medium-selector').val() });
        });
    ...
    prepare_select_source_EnergyUnit: function (ModelBaseID, EnergyMediumID, MeasurePropertyName) {
        ...
        $.ajax({
            url: '/api/EngeryMeasureProperty/GetEnergyAndBalanceUnit',
            type: 'get',
            dataType: 'json',
    ...
    //准备 采集测点 的下拉框数据源
    prepare_select_source_MeterPointID: function (ModelBaseId, EnergyMediumId, MeterpointName) {
        ...
        $.ajax({
            ...
            type: 'get',
            dataType: 'json',
            data: { ModelBaseId: ModelBaseId, EnergyMediumID: EnergyMediumId, MeterpointName: MeterpointName },
            success: function (data) {
                ...
                for (var i in MeterPoint) {
                    ...
                    var option = $('<option>').val(item.MeterPointTagName).html('[' + item.MeterPointTagName + '(' + item.MeterPointInfo + ')]');
    ...
    tree_node_click: function (id, name) {
        ...
        this.my_grid.query({ MeasurePropertyName: $('#search-name').val(), ModelBaseID: id, EnergyMediumId: $('#Medium-selector').val() });
    ...
    window_query: function () {
        ...
        var MeterpointName = $('#MeasurePropertyName_S').val();
        Page.prepare_select_source_EnergyUnit(ModelBaseId, EnergyMediumId, MeasurePropertyName);
        Page.prepare_select_source_MeterPointID(ModelBaseId, EnergyMediumId, MeterpointName);
    },
