var Page = {
    ...
    my_grid_config: {
        ...
        url: '/api/StatisticsReportQuery/GetReportMonthByModelbaseAndUnit',
    ...
    init: function () {
            ...
            success: function (data) {
                if (_.isArray(data)) {
                    for (var i in data) {
                        ...
                        //that.current_endtime = $('#search-quarter1').val();
                    }
                }
            }
        });
            ...
            success: function (data) {
                if (_.isArray(data)) {
                    for (var i in data) {
                        var item = data[i];
                        var option = $('<option>').val(item).html(item);
                        $('#search-quarter3').append(option);
                        //that.current_endtime = $('#search-quarter3').val();
                    }
                }
            }
        });
            ...
            success: function (data) {
                if (_.isArray(data)) {
                    for (var i in data) {
                        var item = data[i];
                        var option = $('<option>').val(item).html(item);
                        $('#search-quarter4').append(option);
                        //that.current_endtime = $('#search-quarter4').val();
                    }
                }
            }
        });
        ...
        $('#query').click(function () {
            that.my_grid.query({
                ...
                monthbegin: $('#search-quarter3').val(),
                monthend: $('#search-quarter4').val(),
            }, that.get_Models);
        });
    ...
    set_current_Models_name_value: function (item) {
        ...
        for (i = 0; i < item.length; i++) {
            Page.current_Models_name[i] = (item[i].Report_StartTime).substr(0, 7); //.replace('T', ' '); //.replace(/-/g, '.').replace(/\//g, '.');
