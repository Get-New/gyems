...
function draw() {
    ...
    if (type == 'day') {
        data = get_consume_detail(tmpModelBaseID, jumpType, 'Day', year, month, day);
    }
    else if (type == 'month') {
        data = get_consume_detail(tmpModelBaseID, jumpType, 'Month', year, month, day);
    }
    else if (type == 'year') {
        data = get_consume_detail(tmpModelBaseID, jumpType, 'Year', year, month, day);
    }
    else {
        return;
    }
...
//查询数据
function get_consume_detail(ModelBaseID, Type, Cycle, Year, Month, Day) {
    var temp_data = null;
    $.ajax({
        url: '/api/OverView/GetCostingDetail',
        method: 'get',
        dataType: 'json',
        async: false,
        data: { ModelBaseID: ModelBaseID, Type: Type, Cycle: Cycle, Year: Year, Month: Month, Day: Day },
...
function create_table(tableData) {
    ...
    if (jumpType == 'Dept') {
        ...
        tmpHeadFirst.append($("<th rowspan='1' colspan='1'>介质种类</th>"));
    }

    tmpHeadFirst.append($("<th rowspan='1' colspan='1'>成本（元）</th>"));
    tmpHeadFirst.append($("<th rowspan='1' colspan='1'>占比</th>"));

    var tmpHeadLast = $("<tr></tr>");
    tmpHeadLast.append($("<th rowspan='1' colspan='2'>合计</th>"));
