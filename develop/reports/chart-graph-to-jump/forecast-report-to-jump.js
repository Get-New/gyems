...
function draw() {
    ...
    if (type == 'day') {
        data = get_forecast_report(tmpModelBaseID, 'Day', year, month, day);
    }
    else if (type == 'month') {
        data = get_forecast_report(tmpModelBaseID, 'Month', year, month, day);
    }
    else if (type == 'year') {
        data = get_forecast_report(tmpModelBaseID, 'Year', year, month, day);
    }
    else {
        return;
    }
...
//查询数据
function get_forecast_report(ModelBaseID, Cycle, Year, Month, Day) {
    var temp_data = null;
    $.ajax({
        url: '/api/OverView/GetForecastReport',
        method: 'get',
        dataType: 'json',
        async: false,
...
function create_table(tableData) {
    ...
    tmpHeadFirst.append($("<th rowspan='1' colspan='1'>预测值</th>"));
    tmpHeadFirst.append($("<th rowspan='1' colspan='1'>单位</th>"));
    tmpHeadFirst.append($("<th rowspan='1' colspan='1'>时间</th>"));
