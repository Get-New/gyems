...
var jumpEnergyMediumID = '';//存放url传参
...
$(function () {
    ...
    //$('#Medium-selector').val(jumpEnergyMediumID);
    set_selected_medium_by_name(jumpEnergyMediumID);//设置跳转介质
...
function get_cvs_by_e() {
    ...
    //只请求输入总量 MeasurePropertyTypes: 1
    if (1) {
        $.ajax({
            ...
            dataType: 'json',
            async: false,
            data: { ModelBaseID: tree_about.current_tree_select_id, EnergyMediumId: $('#Medium-selector').val(), MeasurePropertyTypes: 1, GetCurrentModelBase: true },
            success: function (data) {
                if (!data.Models || !data.Models.length) {
                    return;
                }
...
function set_selected_medium_by_name(currentname) {
    $("#Medium-selector option").each(function () {
        if ($(this).text().indexOf(jumpEnergyMediumID) >= 0) {
...
function get_location_info() {
    ...
    //获得值jumpEnergyMediumID  ModelBaseID:123456,EnergyMediumID:654321 EnergyMediumID:654321 654321
    jumpEnergyMediumID = jumpMSG.split(',')[1].split('EnergyMediumID:')[1];
}