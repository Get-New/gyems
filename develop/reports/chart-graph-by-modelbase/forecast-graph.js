...
$(function () {
    ...
    var tmp_current_year = tmp_current_date.getFullYear();
    var tmp_end_year = tmp_current_year + 3;

    dst.config({
        startYear: 2016,
        startMonth: 1,
        startDay: 1,
        endYear: tmp_end_year,
        endMonth: 12,
        endDay: 31
    })
...
function draw() {
    ...
    if (temp_ids && type == 'month') {
        data = get_line_data_by_cycle(temp_ids, 'Day', year, month);
    }
    else if (temp_ids && type == 'year') {
        data = get_line_data_by_cycle(temp_ids, 'Month', year, month);
    }
    else {
...
//折线图查询
function get_line_data_by_cycle(ids, cycle, year, month) {
    var temp_data = null;
    $.ajax({
        url: '/api/CommonMultiUnit/GetForecastChart',
        method: 'get',
        dataType: 'json',
        async: false,
        data: { UnitId: ids, Cycle: cycle, Year: year, Month: month },
        success: function (data) {
            if (!data.Models || !data.Models.length) {
                return;
            }
