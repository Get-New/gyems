var Page = {
    ...
    my_group_config: function () {
        return {
            ...
            url: '/api/ReportConfig/GetOverViewGraph',
            ps: 0, // 0表示不进行分页（前提是后台未关联分页参数）
    ...
    init: function () {
        ...
        $('#query').click(function () {
            that.my_grid.query({
                ConfigID: that.current_selected_config_id, SubConfigCellName: $('#search-name').val()
            });
        });
        ...
        this.prepare_select_source_SubConfigGroupSix();
    ...
    // 准备所属部门下拉框的数据源
    prepare_select_source_SubConfigGroupSix: function () {
        $('#SubConfigGroupSix').val('').html('');

        $.ajax({
            url: '/api/FactoryModelbase/GetList',
            type: 'get',
            dataType: 'json',
            success: function (data) {
                if (!data.Models || !data.Models.length) {
    ...
    window_open: function () {
        var ModelBaseId = $('#SubConfigGroupSix').val();
