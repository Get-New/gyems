var Page = {
    init: function () {
        current_Reportname1 = '数据统计-天然气-输入量';
        current_Reportname2 = '数据统计-天然气-各车间用量(含铜材)';
        ...
        $('#search-sdate').val(laydate.now(-30, "YYYY-MM"));
        ...
        function get_table_data() {
            ...
            date = today_year + '-' + today_month + '-1';

            //给table中的可变<th>赋值
            //date_title
            $('#date_title1').html(today_year + "年 " + today_month + " 月");
            $('#date_title3').html(today_year + "年 " + today_month + " 月");
            $('#date_title2').html(today_year + "年 1-" + today_month + " 月");
            $('#date_title4').html(today_year + "年 1-" + today_month + " 月");

            //前后台数据交互
            $.ajax({
                url: '/api/ReportCustomize/GetData',
                method: 'get',
                dataType: 'json',
                data: {
                    reportname: current_Reportname1,
                    paramDate: date
                },
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: {
                    reportname: current_Reportname2,
                    paramDate: date
                },
                success: function (data) {
                    if (!data.Models || !data.Models.length) {
