var Page = {
    ...
    init: function () {
        ...
        $('#export').click(function () {
            that.my_grid.exportExcel('绩效日统计');
        })
    ...
    query: function () {
        ...
        $.ajax({
            url: '/api/BenchmarkingRecord/TestGetPage',
            method: 'get',
            data: condition,
            dataType: 'json',
...
function drawTable(data) {
    ...
    // 固定的列
    th = $("<th rowspan='2'>能源基准</th>")   // BenchmarkingName
    th.css('min-width', '150px');
    head1.append(th);

    th = $("<th rowspan='2'>国家指标</th>")   // CountryValue
    th.css('min-width', '100px');
    head1.append(th);

    th = $("<th rowspan='2'>行业指标</th>") // IndustryValue
    th.css('min-width', '100px');
    head1.append(th);

    th = $("<th rowspan='2'>厂内指标</th>") // FactoryValue
    th.css('min-width', '100px');
    head1.append(th);

    th = $("<th rowspan='2'>目标指标</th>") // TargetValue
    th.css('min-width', '100px');
    head1.append(th);
    ...
    for (var i in days) {
        ...
        th = $("<th>实际指标</th>")   // ActualKPIValue
        th.css('min-width', '100px');
        head2.append(th);

        th = $("<th>实现</th>")      // ActualKPIRatio
        th.css('min-width', '100px');
        head2.append(th);
    }
    ...
    for (var i in data.Models) {
        ...
        td = $("<td></td>");
        td.html(line.BenchmarkingName);
        tr.append(td);

        td = $("<td></td>");
        td.html(toFixed(line.CountryValue));
        tr.append(td);

        td = $("<td></td>");
        td.html(toFixed(line.IndustryValue));
        tr.append(td); 

        td = $("<td></td>");
        td.html(toFixed(line.FactoryValue));
        tr.append(td);
