...
var Page = {
    ...
    my_grid_config: {
        store_config: {
            fields: ['RoleId', 'RoleName', 'UserCount']
        },
    ...
    init: function () {
        ...
        $('#query').click(function () {
            that.my_grid.query({ RoleName: $('#search-name').val() });
            that.clear();
        });
    ...
    on_grid_row_delete_clicked: function (record) {
        var that = this;
        var data = { RoleId: record.RoleId };
        Util.confirm('确认要删除名称为' + record.RoleName + '的记录吗?', function () {
            $.ajax({
                url: '/api/ManageRole/Delete',
                type: 'delete',
                data: data,
                success: function (data) {
