var Page = {
    ...
    my_grid_config: {
        store_config: {
            fields: ['FiledirId', 'FiledirName', 'UserCount']
        },
    ...
    init: function () {
        ...
        $('#query').click(function () {
            that.my_grid.query({ FiledirName: $('#search-name').val() });
            that.clear();
        });
        ...
        // 页面加载后先查询一次
        this.my_grid.query({ FiledirName: '' });
    ...
    on_grid_row_delete_clicked: function (record) {
        var that = this;
        var data = { FiledirId: record.FiledirId };
        Util.confirm('确认要删除名称为' + record.FiledirName + '的记录吗?', function () {
            $.ajax({
                url: '/api/ManageFiledir/Delete',
                type: 'delete',
                data: data,
                success: function (data) {
                    if (!data || data.Error) {
